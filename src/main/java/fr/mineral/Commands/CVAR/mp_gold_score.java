        ...
        if(command.getName().equalsIgnoreCase("mp_gold_score")) {
            if(args.length == 1) {
                try {
                    ...
                    mineralcontest.plugin.getServer().broadcastMessage(mineralcontest.prefixGlobal + Lang.cvar_gold_score.toString() + Integer.parseInt(args[0]));
                    return false;
                ...
                sender.sendMessage("[mp_gold_score] Value: " + Game.SCORE_GOLD);
                return true;
            }else {
