package fr.mineral.Core.MapBuilder.Commands;
...
public class listMaps implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(args[0].equals("save")) {
            ...
            Location l = ((Player)sender).getLocation();
            World world =   l.getWorld();
            Location chest = new Location(world, -1327, 61, -2034);
            Location arenaSpawn = new Location(world, -1339, 66, -2034);
            Location top = new Location(world, -1339, 71, -2021);
            Location bottom = new Location(world, -1315, 58, -2046);

            saving = true;
            double x,y,z;
            for(int i = 0; i < 2; i++) {
                for(x = bottom.getX(); x > top.getX(); x--) {
                    for (y = bottom.getY(); y < top.getY(); y++) {
                        for (z = bottom.getZ(); z < top.getZ(); z++) {

                            if(i == 0) {
                                Material[] specialsMaterial = {Material.IRON_DOOR, Material.LADDER};
                                try {
                                    Location tmp = new Location(world, x, y, z);
                                    Block b = tmp.getBlock();
                                    if(b.getType().equals(specialsMaterial[0]) || b.getType().equals(specialsMaterial[1])) {
                                ...
                                try {
                                    ...
                                    if(!b.getType().equals(Material.WATER)) {
                                        ...
                                        //Bukkit.broadcastMessage("NOT SAVING WATER");
                                        tmp.getBlock().setType(Material.AIR);
                                    }
